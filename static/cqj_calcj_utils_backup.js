...
// Debug function to test threshold calculation and validation
window.debugThresholdCalculation = function() {
    console.log('🔍 TESTING THRESHOLD CALCULATION AND VALIDATION');
    ...
    const results = getCombinedIndividualResults();
    if (!results) {
        return { error: 'No individual results available' };
    }
    
    // Find all channels
    const channels = [...new Set(Object.values(results).map(well => well.fluorophore).filter(Boolean))];
    console.log(`📡 Channels found: ${channels.join(', ')}`);
    
    const thresholdResults = {};
    
    channels.forEach(channel => {
        // Get current threshold
        const currentThreshold = window.getChannelThreshold ? 
            window.getChannelThreshold(channel, 'log') : null;
        
        thresholdResults[channel] = {
            current: currentThreshold,
            isValid: currentThreshold > 0 && !isNaN(currentThreshold),
            isNegative: currentThreshold < 0,
            isTooSmall: currentThreshold > 0 && currentThreshold < 0.1
        };
        
        console.log(`📊 ${channel}: ${currentThreshold} (valid: ${thresholdResults[channel].isValid})`);
    });
    ...
    return {
        ...
        validChannels: Object.values(thresholdResults).filter(r => r.isValid).length,
        invalidChannels: Object.values(thresholdResults).filter(r => !r.isValid).length
    };
};

// Chart notification function to show threshold status
window.showThresholdStatus = function(strategy) {
    const debugInfo = window.debugThresholdCalculation();
    if (debugInfo.error) {
        console.warn('Could not get threshold status:', debugInfo.error);
        return;
    }
    
    const { validChannels, totalChannels, thresholdResults } = debugInfo;
    const statusMessage = `${strategy}: ${validChannels} of ${totalChannels} thresholds displayed`;
    
    // Show status in console
    console.log(`🎯 THRESHOLD-STATUS - ${statusMessage}`);
    
    // Optional: Show visual indicator on chart if needed
    if (validChannels < totalChannels) {
        const invalidChannels = Object.entries(thresholdResults)
            .filter(([channel, result]) => !result.isValid)
            .map(([channel, result]) => `${channel}(${result.current})`);
        
        console.warn(`⚠️ THRESHOLD-STATUS - Invalid thresholds: ${invalidChannels.join(', ')}`);
    }
    
    return { statusMessage, validChannels, totalChannels, invalidChannels: totalChannels - validChannels };
};

// Test function to simulate threshold issues for debugging
window.testThresholdIssues = function() {
    console.log('🧪 TESTING THRESHOLD ISSUES');
    
    // 1. Test current threshold status
    console.log('1. Current threshold status:');
    const status = window.showThresholdStatus('test');
    
    // 2. Force threshold recalculation
    console.log('2. Forcing threshold update...');
    if (window.updateAllChannelThresholds) {
        window.updateAllChannelThresholds();
    }
    
    // 3. Check if negative thresholds exist
    console.log('3. Checking for problematic thresholds...');
    const debugInfo = window.debugThresholdCalculation();
    if (!debugInfo.error) {
        const problematicChannels = Object.entries(debugInfo.thresholdResults)
            .filter(([channel, result]) => result.isNegative || result.isTooSmall)
            .map(([channel, result]) => ({ 
                channel, 
                value: result.current, 
                issue: result.isNegative ? 'NEGATIVE' : 'TOO_SMALL' 
            }));
        
        if (problematicChannels.length > 0) {
            console.warn('❌ Problematic thresholds found:', problematicChannels);
        } else {
            console.log('✅ All thresholds appear valid');
        }
    }
...
// Force chart visual refresh to ensure thresholds are displayed
window.forceChartThresholdDisplay = function() {
    try {
        ...
        // Force threshold recalculation and display
        if (window.updateAllChannelThresholds) {
            window.updateAllChannelThresholds();
        }
        
        // Delayed second attempt for timing issues
        setTimeout(() => {
            if (window.amplificationChart && typeof window.amplificationChart.update === 'function') {
                window.amplificationChart.update();
            }
